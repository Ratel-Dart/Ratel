...
/// Writes `.dart_tool/ratel/bootstrap.dart` and returns its path.
///
/// The bootstrap registers every generated library and then calls the
/// application's own `main`. It lives under `.dart_tool` — a `file:` URI —
/// precisely so it can reach both `package:` libraries under `lib/` and plain
/// files under `bin/`, which a generated library in `lib/` could not do.
String writeBootstrap(RatelProject project, File entrypoint) {
  ...
  file.writeAsStringSync(
    '// GENERATED BY THE RATEL CLI - DO NOT MODIFY BY HAND\n'
    '$imports\n'
